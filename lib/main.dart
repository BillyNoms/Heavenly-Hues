    ...
    return MaterialApp(
      ...
      theme: ThemeData(
        ...
        dialogTheme: DialogTheme(
          backgroundColor: Color(0xFF121212), // Updated color for dialog background
        ),
      ),
...
class _BibleVerseScreenState extends State<BibleVerseScreen> {
  ...
  _navigateToVerse() {
    if (_searchController.text.isNotEmpty) {
      ...
      // Updated dialog box with the required colors and styles
      showDialog(
        context: context,
        builder: (context) => AlertDialog(
          backgroundColor: Color(0xFF121212), // Background color of the dialog
          title: Text('Empty Verse Reference', style: TextStyle(color: Colors.white)), // Font color set to white
          content: Text('Please enter a verse reference to search.', style: TextStyle(color: Colors.white)), // Font color set to white
          actions: [
            TextButton(
              onPressed: () => Navigator.pop(context),
              child: Text('OK', style: TextStyle(color: Colors.teal[400])), // OK button color set to teal[400]
            ),
          ],
        ),
      );
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('Heavenly Hues', style: GoogleFonts.getFont('Cookie').copyWith(fontSize: 30)),
  ...
  Widget _buildVerseOfTheDay() {
    return Container(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.center,
        children: [
          Text('Verse of the Day', style: GoogleFonts.getFont('Libre Baskerville').copyWith(fontSize: 18, color: Colors.white)),
          SizedBox(height: 18),
        ...
        children: [
          ...
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text('Embrace the stories of faith, hope, and redemption to ignite a new passion within.', style: GoogleFonts.getFont('Libre Baskerville').copyWith(fontSize: 15, color: Colors.white)),
                SizedBox(height: 5),
                Text('— let the Bible speak to your heart today.', style: GoogleFonts.getFont('Libre Baskerville').copyWith(fontSize: 14, fontStyle: FontStyle.italic, color: Colors.white)),
                SizedBox(height: 15),
                Text('About:\nBible Project creates free resources to help you experience the Bible. You can see our entire library of videos, podcasts, and classes, and other resources at bibleproject.com.', style: GoogleFonts.getFont('Libre Baskerville').copyWith(fontSize: 13.3, color: Colors.white70)),
                SizedBox(height: 10),
                ElevatedButton(
      ...
      child: Row(
        children: [
          Expanded(
            child: TextField(
              ...
              decoration: InputDecoration(
                ...
                hintStyle: GoogleFonts.libreBaskerville().copyWith(color: Colors.white70, fontSize: 12),
          ...
          ElevatedButton(
            ...
            child: Text('Search', style: TextStyle(color: Colors.white, fontFamily: 'Libre Baskerville')),
            style: ElevatedButton.styleFrom(primary: Colors.teal, onPrimary: Colors.white, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(20.0))),
          ),
        ],
      ),
  ...
  Widget _buildBookAbbreviationsContainer() {
    return GestureDetector(
      ...
      child: Container(
        ...
        child: Center(
          child: Text('Tap for Bible book abbreviations', style: GoogleFonts.getFont('Libre Baskerville').copyWith(fontSize: 14, color: Colors.white, fontStyle: FontStyle.italic)),
        ),
      ),
    );
  }
                ...
                builder: (context, snapshot) {
                  if (snapshot.connectionState == ConnectionState.waiting) {
                    ...
                    return Center(child: Column(
                      ...
                      children: [
                        Image.asset('image/biblelogo.png', height: 200, width: MediaQuery.of(context).size.width * 1, fit: BoxFit.cover
                        ),
                        Text('\n${snapshot.error}. Input a valid one, refer to the suggested Bible book abbreviations.', style: TextStyle(color: Colors.black, fontSize: 16, fontStyle: FontStyle.italic)),
                      ],
                    ),
                    );
                  } else {
                    return Column(
                      crossAxisAlignment: CrossAxisAlignment.center,
                      children: [
                        Text(snapshot.data?['text'] ?? 'Verse not found', textAlign: TextAlign.center, style: GoogleFonts.libreBaskerville().copyWith(fontSize: 16)),
                        SizedBox(height: 10),
                        Text(snapshot.data?['reference'] ?? 'Reference not found', textAlign: TextAlign.center, style: GoogleFonts.libreBaskerville().copyWith(fontSize: 14, fontStyle: FontStyle.italic)),
                      ],
                    );
                  }
                },
